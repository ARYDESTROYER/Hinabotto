...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const mess = {
            ...
            error: {
                St: '[❗] Send an image with the caption * # sticker * or tag of the image that has been sent',
                Ti: '[❗] Replay the sticker with the caption * # stickertoimg * or the sticker tag that has been sent',
                Qm: '[❗] An error occurred, the theme may be unavailable!',
                ...
                Ki: '[❗] Bot cant remove group Admin!',
                Sp: '[❗] Bot cant remove Admin',
                Ow: '[❗] Bot cant remove Owner',
                Bk: '[❗] Bot cant block Owner',
        ...
        const ownerNumber = '917038702698@c.us'
        ...
        // FUNCTION
                function isMsgLimit(id){
                    ...
                    for (let i of msgLimit){
                        if(i.id === id){
                            if (i.msg >= 12) {
                                found === true 
                                tobz.reply(from, '*[ANTI-SPAM]* \ nSorry, your account is blocked due to SPAM, and cannot be UNBLOCKED!', id)
                                ...
                                found === true
                                tobz.reply(from, '*[ANTI-SPAM]* \nYou may be spamming, sending 5 more messages in an unreasonable time will automatically block you.', id)
                                return true
                            }else{
                                found === true
                                return false;
                    ...
                    for (let i of limit){
                        if(i.id === id){
                            ...
                            if (limits >= limitCount) {
                                found = true;
                                tobz.reply(from, 'Bot quota exhausted, try again later :)', id)
                                return true;
                            }else{
                                limit
                                found = true;
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isGroupAdmins) return tobz.reply(from, 'Sorry, this command can only be executed by group admins !', id)
                        ...
                        tobz.reply(from, 'Bot has been muted in this chat, "#unmute" to unmute !', id)
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                        ...
                        muted.push(chatId)
                        fs.writeFileSync('./lib/muted.json', JSON.stringify(muted, null, 2))
                        reply(from, 'Bot has been muted in this chat, "#unmute" to unmute !', id)
                    }
                }
                if(body === '#unmute' && isMuted(chatId) == false){
                    if(isGroupMsg) {
                        ...
                        fs.writeFileSync('./lib/muted.json', JSON.stringify(muted, null, 2))
                        tobz.reply(from, 'Bot was unmuted successfully !', id)         
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Apologies, command can only be executed by bot admins !', id)
                    if(setting.banChats === false) return
                    setting.banChats = false
                    banChats = false
                    fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null, 2))
                    tobz.reply('Global chat has been disabled!')
                }
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Command can only be executed by bot admn.', id)
            setting.banChats = true
            banChats = true
            fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null, 2))
            tobz.reply('Global chat has been enabled!')
            break
        ...
        case '#stiker':
            ...
            break
        case '#ttp':
                if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chats !', message.id)
                try
                {
                    ...
                    if(args)
                    {
                        if(quotedMsgObj == null)
                        {
                            ...
                            if(gasMake.status == true)
                            {
                                try{
                                    ...
                                    await tobz.reply(from, 'Failed.', id)
                                } 
                            }else{
                        ...
                        await tobz.reply(from, 'Text cannot be empty.', id)
                    }
                }catch(error)
                {
                    console.log(error)
        ...
        case '#sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    tobz.reply(from, '[PROCESSING] ⏳ please wait ± 1 min!', id)
                    ...
                    tobz.reply(from, '[❗] Send video with caption *#stickerGif* max 10 sec!', id)
                )
            }
            break
        case '#stickerfire':
                if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chats !', id)
                tobz.reply(from, '[PROCESSING] ⏳ please wait ± 1 min!', id)
                if (isMedia && type === 'image') {
                const mediaData = await decryptMedia(message, uaOverride)
                ...
                await tobz.reply(from, 'Wrong Format!\n⚠️ Please send smages With #stickerfire', id)
                }
                break
        case '#toimg':
            ...
            } else if (!quotedMsg) return tobz.reply(from, 'Please tag the sticker that you want to use as an image!', id)
            break
        case '#groupinfo' :
            if (!isGroupMsg) return tobz.reply(from, 'Apologies, command can only be executed in groups !', message.id)
...
${desc}`)
        ...
        case '#quoterandom' :
        case '#quote' :
            if (!isGroupMsg) return tobz.reply(from, 'Apologies, command can only be executed in groups !', id)
            if (isLimit(serial)) return tobz.reply(from, `Apologies ${pushname}, you have reached your daily quota, "#limit" to check quota.`, id)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `917038702698@c.us`)
            tobz.reply(from, 'For all queries, contact the above mentioned number!', id)
            break
        // ON OFF
        case '#nsfw':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in a group chat!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'You must be a group admin to enable this!', id)
            if (args.length === 1) return tobz.reply(from, 'Enable or disable ???', id)
            if (args[1].toLowerCase() === 'enable') {
                nsfw_.push(chat.id)
                fs.writeFileSync('./lib/nsfwz.json', JSON.stringify(nsfw_))
                tobz.reply(from, 'NSFW has been successfully activated, perverts.....', id)
            } else if (args[1].toLowerCase() === 'disable') {
                nsfw_.splice(chat.id, 1)
                fs.writeFileSync('./lib/nsfwz.json', JSON.stringify(nsfw_))
                tobz.reply(from, 'NSFW has been successfully disabled!', id)
            } else {
                tobz.reply(from, 'Enable or disable ??', id)
            }
            break
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chat !', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Only bot admins can perform this task !', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Enable or disable ???', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
                fs.writeFileSync('./lib/Simsimi.json', JSON.stringify(simi_))
                tobz.reply(from, 'Simsimi has been successfully activated on this group! Send command *# [text]* \nExample: *# hello* (There must be a space bw # and the text)', id)
            } else if (args[1].toLowerCase() === 'disable') {
                simi_.splice(chat.id, 1)
                fs.writeFileSync('./lib/Simsimi.json', JSON.stringify(simi_))
                tobz.reply(from, 'Simsimi has successfully been deactivated in this group!', id)
            } else {
                tobz.reply(from, 'Enable or disable ???', id)
            }
            break
        case '#group':
            ...
            if (args[1].toLowerCase() === 'open') {
                ...
                tobz.reply(from, 'Open or close ???', id)
            }
            break
        case '#left':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '#loli':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chats !', id)
            if (isLimit(serial)) return tobz.reply(from, `Apologies ${pushname}, you have reached your daily quota, to check, type "#limit"`, id)
        ...
        case '#waifu':
            ...
            break
        case '#husbu':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be exected in group chat !', id)
            if (isLimit(serial)) return tobz.reply(from, `Apologies ${pushname}, you have reached your daily quota, to check, type "#limit"`, id)
        ...
        case '#randomtrapnime':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chat !', id)
            if (!isNsfw) return tobz.reply(from, 'NSFW must be activated in order to view this content!', id)
            if (isLimit(serial)) return tobz.reply(from, `Apologies ${pushname}, you have reached your daily quota, to check, type "#limit"`, id)
        ...
        case '#nhder':
            ...
            if (args.length >=2){
                ...
                tobz.sendText(from, 'Please enter the nuclear code in order to verify your identity')
            }
            break
        /*case '#wallanime' :
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '#quoteanime':
            ...
            break
        case '#anime':
            if (!isGroupMsg) return tobz.reply(from, 'Apologies, this command can only be executed in groups', id)
            if (isLimit(serial)) return tobz.reply(from, `Sorry,${pushname}, you have reached your daily quota, "#limit" to check your quota limit.`, id)
           ...
           `https://api.jikan.moe/v3/search/anime?q=${keyword}`
            ...
            if (!parsed) {
              await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Error, anime not found !', id)
              return null
              }
...
🌐️ *URL*: ${url}`
            ...
            tobz.sendImage(from, base64, title, content)
             ...
             console.error(err.message)
             await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Error, anime not found !')
           }
          break
        case '#character':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in a group chat!', id)
           ...
           `https://api.jikan.moe/v3/search/character?q=${keywords}`
            ...
            const parsed = await data.json()
            if (!parsed) {
              await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Error, character not found !', id)
              return null
              }
            const { name, alternative_names, url, image_url } = parsed.results[0]
            const contentt = `*Character found !*
...
🌐️ *URL*: ${url}`
            ...
            tobz.sendImage(from, base64, name, contentt)
             ...
             console.error(err.message)
             await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Error, character not found !')
           }
          break
        // MEDIA //
        case '#infogempa':
        ...
        case '#ytmp3':
            ...
            break
        case '#google':
            if (!isGroupMsg) return tobz.reply(from, 'This command must be executed in group chats !', id)
            if (isLimit(serial)) return tobz.reply(from, `Apologies ${pushname}, you have reached your daily quota, to check, type "#limit"`, id)
            
            await limitAdd(serial)
            tobz.reply(from, mess.wait, id)
            const googleQuery = body.slice(8)
            if(googleQuery == undefined || googleQuery == ' ') return tobz.reply(from, `*Search result : ${googleQuery}* Err. Not found !`, id)
            ...
            if(args.length >= 2){
                ...
                translatte(text, {to: codelang}).then(res => {
                    ...
                    limitAdd(serial)
                     ...
                     tobz.sendText(from,`[ERROR-404] Text/Language code ${codelang} not found ! Type "#languagecode" to view.`);
                });
            }
            break
        case '#video':
...
➸ *Total Rekaman:* ${recording}`
        ...
        case '#':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in a group chat !', id)
            if (!isSimi) return tobz.reply(from, 'NSFW must be activated in order view this content !', id)
            if (args.length === 1) return tobz.reply(from, 'Err. command usage *# [Text here]* \nExample *# Hello*')
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#nhentai': // NHENTAI PREMIUM BY TOBZ
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND', id)
          break
        /*case '#nhentai':
        case '#nh':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '#math':
            if (!isGroupMsg) return tobz.reply(from, 'Apologies, this command can only be executed in groups !', id)
            if (args.length === 1) return tobz.reply(from, '[❗] Send command "*#math 1+1*"')
            const mtk = body.slice(6)
            if (typeof Math_js.evaluate(mtk) !== "number") {
            tobz.reply(from, `"${mtk}", is NOT a number.`, id)
        } else {
        ...
        case '#quotemaker':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chats !', id)
            if (isLimit(serial)) return tobz.reply(from, `Apologies ${pushname}, your daily quota has exhausted, to check "#limit"`, id)
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotes, author, theme).then(amsu => {
                    tobz.sendFile(from, amsu, 'quotesmaker.jpg','meh...').catch(() => {
                       tobz.reply(from, mess.error.Qm, id)
                    })
                })
            } else {
                tobz.reply(from, 'Usage: \n#quotemaker |Text/quote|watermark|theme\n\nEx :\n#quotemaker |Stay hungry, stay foolish|Ary|random', id)
            }
            break
        case '#jadwalshalat':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in group chats !', id)
        ...
        case '#nulis':
            ...
            break
        case '#qrcode':
           if (!isGroupMsg) return tobz.reply(from, 'This command can only be executed in group chats!', id)
        if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case '#broadcast':
            if (!isOwner) return tobz.reply(from, 'You must be bot owner to perform this task !', id)
            let msg = body.slice(10)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) await tobz.sendText(ids, `[ BOT BROADCAST ]\n\n${msg}`)
            }
            tobz.reply(from, 'Broadcast transmission successful !', id)
            break
        case '#adminlist':
            if (!isGroupMsg) return tobz.reply(from, 'This command can only be executed in group chats!', id)
            let mimin = ''
        ...
        case '#mentionall':
            if (!isGroupMsg) return tobz.reply(from, 'This command can only be executed in group chats!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'You must be group admin to perform this task !', id)
        ...
        case '#okickall':
            ...
            break
        case '#kickall':
            if (!isGroupMsg) return tobz.reply(from, 'This command must be executed in a group chat!', id)
            const isGroupOwner = sender.id === chat.groupMetadata.owner
            if (!isGroupOwner) return tobz.reply(from, 'You must be the group owner in order to perform this task !', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Bot must be group admin in order to perform this task !', id)
        ...
        case '#leaveall':
            ...
            break
        case '#clearall':
            if (!isOwner) return tobz.reply(from, 'You must be bot admin in order to perform this task !', id)
            const allChatz = await tobz.getAllChats()
            for (let dchat of allChatz) {
                await tobz.deleteChat(dchat.id)
            }
            tobz.reply(from, 'Successfully cleared all chat logs!', id)
            break
        case '#oadd':
            const orang = args[1]
            if (!isGroupMsg) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
        ...
        case '#demote':
            ...
            break
        case '#join':
            if (args.length === 1) return tobz.reply(from, 'Only the Bot Admin can perform this task!', id)
            if (!isOwner) return tobz.reply(from, 'Only Bot Admin can perform this task!', id)
            const link = body.slice(6)
            const tGr = await tobz.getAllGroups()
            const minMem = 5
            const isLink = link.match(/(https:\/\/chat.whatsapp.com)/gi)
            const check = await tobz.inviteInfo(link)
            if (!isLink) return tobz.reply(from, 'WHERE IS THE LINK, BAKA??? 👊🤬', id)
            if (tGr.length > 256) return tobz.reply(from, 'Sorry, I am at my group limit!', id)
            if (check.size < minMem) return tobz.reply(from, 'Group must have atleast 5 members in order for the bot to function.', id)
            if (check.status === 200) {
                await tobz.joinGroupViaLink(link).then(() => tobz.reply(from, 'The bot will join the group soon!'))
            } else {
                tobz.reply(from, 'Group link invalid !', id)
            }
            break
        case '#odelete':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
        ...
        case '#delete':
            if (!isGroupMsg) return tobz.reply(from, 'Command must be executed in a group chat !', id)
            if (!isGroupAdmins) return tobz.reply(from, 'You must be a group admin in order to perform this task !', id)
            if (!quotedMsg) return tobz.reply(from, 'Wrong !! Send command *#delete [tagbot]*', id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, 'NOO ! Bots cannot delete other users chats !!', id)
            tobz.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
        case '#getses':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner Elaina', id)            
        ...
        case '#limit':
            ...
            for(let lmt of limidat){
                if(lmt.id === serial){
                    ...
                    if(limitCounts <= 0) return tobz.reply(from, `Your daily quota has exhausted\n\n_Note : Quota is resetted at 2100 daily!_`, id)
                    tobz.reply(from, `Daily request quota available : *${limitCounts}*\n\n_Note : Quota is resetted at 2100 daily!_`, id)
                    found = true
                }
            }
            console.log(limit)
            ...
            if (found === false){
                ...
                tobz.reply(from, `Daily request quota available : *${limitCount}*\n\n_Note : Quota is resetted at 2100 daily!_`, id)
            }
            break
        case '#restart':
            if(isOwner){
                tobz.sendText(from, '*[WARNING]* Restarting ...')
            ...
            break
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'You must be bot admin to perform this task!', id)
                for (let i = 0; i < mentionedJidList.length; i++) {
                adminNumber.push(mentionedJidList[i])
                fs.writeFileSync('./lib/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Admin added successfully!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'You must be bot admin to perform this task!', id)
                ...
                fs.writeFileSync('./lib/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Admin removed successfully!', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'You must be bot admin to perform this task!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                ...
                await tobz.contactBlock(unblock).then((a)=>{
                    console.log(a)
                    tobz.reply(from, `Blocked successfully ${args[1]}!`, id)
                })
            }
            break
        case '#unblock':
            ...
            for (let i = 0; i < mentionedJidList.length; i++) {
                ...
                await tobz.contactUnblock(unblock).then((a)=>{
                    console.log(a)
                    tobz.reply(from, `Successfully unblocked ${args[1]}!`, id)
                })
            } 
            break
        case '#ban':
            if (!isAdmin) return tobz.reply(from, 'You must be bot admin to perform this task!', id)
                for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./lib/banned.json', JSON.stringify(banned))
                tobz.reply(from, 'Target user banned successfully!',id)
            }
            break
        case '#unban':
            if (!isAdmin) return tobz.reply(from, 'You must be bot admin to perform this task!', id)
                ...
                banned.splice(inz, 1)
                fs.writeFileSync('./lib/banned.json', JSON.stringify(banned))
                tobz.reply(from, 'Target user unbanned succesfully!', id)
            break
        case '#listgroup':
        ...
        case '#ping':
            ...
            tobz.sendText(from, `RAM in use: *${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB*\nCPU: *${os.cpus().length}*\n\nStatus :\n- *${loadedMsg}* Loaded Messages\n- *${groups.length}* Group Chats\n- *${chatIds.length - groups.length}* Personal Chats\n- *${chatIds.length}* Total Chats\n\nSpeed: ${latensi.toFixed(4)} _Second_`)
            break
        case '#bugreport':
        ...
        case '#help':
            ...
            break
        case '#supportgroup':
            tobz.reply(from, `Bot support group url: https://chat.whatsapp.com/Eq2tH6bYBwfAVbzNZKPDhT Join for help and suggestions regarding bot ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya untuk Admin Elaina', id)
        ...
        case '#ownermenu':
            ...
            break
        case '#nsfwmenu':
            if (!isGroupMsg) return tobz.reply(from, 'This command can only be executed in a group !', id)
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
        ...
        case '#tnc':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, '----------', id)
            if (command.startsWith('#')) {
                tobz.reply(from, `${pushname}, "*${args[0]}*" is not a listed command, BAKA !!`, id)
            }
            await tobz.sendSeen(from) 
        }
